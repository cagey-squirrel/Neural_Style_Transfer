...
class Vgg19_nm(torch.nn.Module):
    """
    ...
    def __init__(self, requires_grad=False, show_progress=False, use_relu=True):
        super().__init__()
        vgg_pretrained_features = vgg19(weights=torchvision.models.VGG19_Weights.IMAGENET1K_V1, progress=show_progress).features
    ...
    def forward(self, x):
        print(f'req grad = {x.requires_grad}')
        x = self.slice1(x)
        layer1_1 = x
        x = self.slice2(x)
        layer2_1 = x
